...
int main(int, char*[])
{
	...
	auto ptrE1 = std::make_shared<Ellipsoid>("w1", State(position_o1), Eigen::Array3d(0.1, 0.5, 0));
	...
	for(unsigned int i=0; i<nb_steps; ++i)
	{
		...
		// obstacle
		Eigen::Vector3d obstacle_current_position = env["w1"]->get_position();
		...
		env["w1"]->set_linear_velocity(obstacle_desired_velocity);
		...
		env["w1"]->set_position(obstacle_position);
		env["w1"]->set_reference_position(obstacle_position);
